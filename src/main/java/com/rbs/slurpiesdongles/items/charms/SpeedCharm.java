...
public class SpeedCharm extends CharmBase {
    ...
    public boolean onTick(ItemStack stack, EntityPlayer living, World world) {
        if (!this.canTick(stack)) {
            living.addPotionEffect(new PotionEffect(MobEffects.SPEED, 600, 0, false, false));
        }
        return false;
    }
    @Override
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        tooltip.add(new TextComponentString("Having this item in your inventory grants you Speed"));
    }
}
