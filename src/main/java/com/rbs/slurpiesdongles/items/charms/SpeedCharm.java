...
public class SpeedCharm extends CharmBase {
    ...
    public boolean onTick(ItemStack stack, PlayerEntity living, World world) {
        if (!this.canTick(stack)) {
            living.addPotionEffect(new EffectInstance(Effects.SPEED, 600, 0, false, false));
        }
        return false;
    }
    @Override
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<ITextComponent> tooltip, ITooltipFlag flagIn) {
        tooltip.add(new StringTextComponent(TextFormatting.YELLOW + "Having this item in your inventory grants you Speed"));
    }
